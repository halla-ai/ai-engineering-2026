---
title: "Week 9: Implementing the Planner and QA Agents"
description: From the spec-generating planner to the independent QA agent — implementing the two core pipeline agents
week: 9
phase: 3
phase_title: Multi-Agent SDLC
date: "2026-04-28"
theory_topics:
  - Planner agent architecture and the 2-phase separation pattern
  - QA agent role and independence principle
  - Automated test pipeline and feedback loops
lab_topics:
  - Planner agent + spec.md generation pipeline
  - QA agent + 3-parallel reviewer
assignment: "Lab 09: Planner + QA Agent Implementation"
...
<Aside type="tip" title="Key claim">
The planner agent is the pipeline's first gate. **Spec quality = overall pipeline success rate.** Investing in the planner has a higher ROI than upgrading the coder agent.
</Aside>
...
<Aside type="caution" title="A bad spec poisons everything downstream">
Even correctly-implemented code is useless if the spec misunderstood the requirement. This is why the QA agent in the next stage validates **only the code and tests, not the spec** — independently.
</Aside>
